...
class   AccountsControllerTest {
  ...
  void transfer_success() throws Exception {
    ...
    Account account1 = new Account("1", new BigDecimal("423.45"));
    Account account2 = new Account("2", new BigDecimal("213.95"));
  ...
  void transfer_accountNotFound() throws Exception {
    ...
    this.mockMvc.perform(post("/v1/accounts//transfer")
                    ...
                    .content("{\"accountFromId\":1,\"accountToId\":2,\"amount\":100}"))
            .andExpect(status().isBadRequest())
            .andExpect(content().string("Account not found: 1"));
  }

  @Test
  void transfer_insufficientFunds() throws Exception {
    Account account1 = new Account("1", new BigDecimal("23.45"));
    Account account2 = new Account("2", new BigDecimal("213.95"));
    this.accountsService.createAccount(account1);
    this.accountsService.createAccount(account2);
    mockMvc.perform(post("/v1/accounts//transfer")
                    ...
                    .content("{\"accountFromId\":1,\"accountToId\":2,\"amount\":100}"))
            .andExpect(status().isBadRequest())
            .andExpect(content().string("Insufficient funds in account: 1"));
  }
